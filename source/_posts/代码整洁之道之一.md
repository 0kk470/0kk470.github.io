...
### 4. 可读性良好的命名
***
* 建议试用英文单词进行变量命名，不要用各种简写或者拼音，如以下为不好的命名示例。
```CSharp
    class DtaRcrd102
    {
...
### 5. 可搜索命名
***
尽可能地使用常量定义和单词变量地替代纯数字或者纯字符串，这样其他人想修改值的时候只需要搜索到定义的地方修改就行，不用一个个去找。比如下面这段代码
...
```
这样当后续想要修改最大武器数量只需要修改一处即可，也便于维护者按关键字```max```之类的搜索。
...
* 不要在不合适的情况下使用双关语。比如 ```Add``` 函数这种，在一个类当中他可能表示将两个值相加，也可能表示将一条数据条目添加到指定集合当中，如果出现了这种情况可以考虑换成```Append```或者```Insert```。
...
***
* 添加有意义的语境
  * 一个函数中出现了```state```变量，阅读者的最开始理解不了这个变量的意义，于是他就得去定向到这个state的定义，结合定义时的上下文来理解，降低了熟悉代码的时间效率。
...
* 避免无意义的语境
  * 比如在某些类前面加上了应用名称的前缀，比如一个```加油站豪华版应用(Gas Station Deluxe)```中的每个类都加了```GSD```前缀，类似```GSDAccountAddress```这种，本来只是一个表示右键地址的类，加了前缀后会让其他新进的开发人员误以为是用来什么特殊处理逻辑。
  * 这个问题个人也在公司内遇见过，在阅读前东家游戏项目的C++源码时发现了很多带ES前缀的类，比如ESArray、ESString之类的，读下来发现只是一些针对项目本身需求优化过的STL容器，一直很疑惑ES是啥特殊含义，还跑去问了上级老大，结果被告知ES是公司的英文简写。 这种容易造成误解的情况还是尽可能避免，实在需要的话，可以用命名空间包一层来处理。
